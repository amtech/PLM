...
class Ion_auth
{
	...
	public function __construct()
	{
		...
		if ($this->logged_in()) {
            ...
            define("USER_IMAGE", $user->user_image);
			define("USER_NAME", $user->first_name." ".$user->last_name);
			define("USER_ID", $user->id);

			// Site setting
			$SETTING = $this->ion_auth_model->get_setting();
            define("DEFAULT_BRANCH", $SETTING->default_branch_id);
			define("LOGO", $SETTING->logo);
			define("SITE_NAME", $SETTING->system_name);
			define("CURRENCY", $SETTING->currency_code);
			define("QUOTATION_PREFIX", $SETTING->quotation_prefix);
			define("DEFAULT_DISCOUNT", $SETTING->default_discount);
            define("ALERT_NO", $this->ion_auth_model->get_total_results());
            define("REPORT_COMPLETED",$this->ion_auth_model->getCompletedReport());
            define("REPORT_PENDING",$this->ion_auth_model->getPendingReport());
		}
	}
