...
class Ion_auth_model extends CI_Model
{
	...
	**/
    ...
    {
		$this->db->where('warehouse_items.quantity <= parts.alert_quantity',NULL,FALSE)
        ->from('parts')
    ...
    public function getCompletedReport(){
        $q = $this->db->select('commission.*,services.technician_name,records.ro_no')
            ->from('commission')
    ...
    function updateBranch($identity){
        $q = $this->db->get_where('users',array('email'=>$identity));
        // echo $this->db->last_query();exit;
        if($q->num_rows() > 0){
            $row = $q->row();
            $branch = $row->branch_id;
            $q_currency = $this->db->get_where('branch',array('id'=>$branch));
            // echo $this->db->last_query();exit;
            $row_currency = $q_currency->row();
            $currency = $row_currency->currency_code;
            $this->db->update('settings',array('currency_code'=>$currency));
            $this->db->where('id',1);
            if($this->db->update('settings',array('default_branch_id'=>$branch))){
                // echo $this->db->last_query();exit;
                return true;
            }
        }
