...
class Pdc extends CI_Controller 
{
    ...
    function index()
    {
        ...
        $data['pdc'] = $this->pdc_model->getPDC();
        // echo '<pre>';
        // print_r($data['pdc']);exit;
    ...
    {
        $this->datatables->select('pdc.id,branch.branch_name as branchName,date_issue,date_cheque,cheque_no,bank_ref,party_favouring,reason,amount,bank_account_no')
        //->unset_column('pdc.id')
        ->from('pdc')
    ...
    function add()
    {
        ...
        {
            $issue_date = explode('/',$this->input->post('date_issue'));
            $original_issue_date = $issue_date[2].'-'.$issue_date[1].'-'.$issue_date[0];
            
            $check_date = explode('/',$this->input->post('date_cheque'));
            $original_check_date = $check_date[2].'-'.$check_date[1].'-'.$check_date[0];
            
            $from_date = explode('/',$this->input->post('payment_from_date'));
            $original_from_date = $from_date[2].'-'.$from_date[1].'-'.$from_date[0];
            
            $to_date = explode('/',$this->input->post('payment_to_date'));
            $original_to_date = $to_date[2].'-'.$to_date[1].'-'.$to_date[0];
            
            $pdcDetail = array(
                    'branch_id'	        =>	$this->input->post('branch'),
                    'date_issue'	    =>	$original_issue_date,
                    'date_cheque'       =>  $original_check_date,
                    ...
                    'payment_from_date' =>  $original_from_date,
                    'payment_to_date'   =>  $original_to_date,
