        ...
        <script type="text/javascript">
                ...
                $('#add').click(function(){
                    ...
                    $('#myTable tr:last').after(
                        '<tr id='+count+'>'+
                            ...
                            '<input type="hidden" name="total_price_hidden'+count+'" id="total_price_hidden-'+count+'">'+
                        '</tr>'
                    );
                    $('.drop').chosen(); 
            ...
            function calculateSubTotal(object){
                ...
                $("#total_price_hidden-"+count).val(total_price);
            ...
            <div class="col-lg-12">
                ...
                <section class="panel">
                    ...
                    <div class="panel-body">
                        <form class="form-horizontal" role="form" action="<?php echo base_url(); ?>index.php/quotations/edit/<?php echo $id; ?>" method="post" id="quotation_form">
                            <div class="form-group">
                                ...
                                <div class="col-lg-4">
                                <?php
                                    $optionCustomer = array('0'=>'Select Company');
                                    if(!empty($customers)){
                                        foreach($customers as $rowCustomer){
                                            $optionCustomer[$rowCustomer->id] = $rowCustomer->name;
                                        }
                                    }
                                    echo form_dropdown('customer_id',$optionCustomer,isset($_POST['customer_id'])?$_POST['customer_id']:$quotation->customer_id,'class="form-control chosen-select" id="customer_id"');
                                ?>
							...
							<div class="form-group">
                                ...
                                <div class="col-lg-4">
                                <?php
                                    $optionBranch = array('0'=>'--Select--');
                                    if(!empty($branch)){
                                        foreach($branch as $rowBranch){
                                            $optionBranch[$rowBranch->id] = $rowBranch->branch_name;
                                        }
                                    }
                                    echo form_dropdown('branch_id',$optionBranch,isset($_POST['branch_id'])?$_POST['branch_id']:$quotation->branch_id,'class="form-control" id="branch_id"');
                                ?>  
                                </div>
                                
                                <label for="quotation_date" class="col-sm-2 control-label col-lg-2">Quotation Date</label>
                                <div class="col-lg-4">
                                    <input class="form-control form-control-inline input-medium default-date-picker" type="text" id="quotation_date" placeholder="Select Quotation Date" name="quotation_date" value="<?php echo set_value('quotation_date',date("d/m/Y", strtotime($quotation->quotation_date))); ?>" autocomplete="off" />
                                </div>
                            </div>
                            <div class="form-group">
                                <label for="validity" class="col-sm-2 control-label col-lg-2">Validity Upto</label>
                                <div class="col-lg-4">
                                    <input type="text" class="form-control" id="validity" placeholder="Validity Upto" name="validity" value="<?php echo set_value('validity',$quotation->validity); ?>">
                                </div>
                            ...
                            <div class="form-group">
								...
								<div class="col-lg-4">
									<input type="text" id="delivery_place" name="delivery_place" class="form-control" value="<?php echo set_value('delivery_place',$quotation->delivery_place); ?>" />
								</div>
								
								<label class="col-sm-2 control-label col-lg-2" for="payment_terms">Payment Term</label>
								<div class="col-lg-4">
									<input type="text" name="payment_terms" id="payment_terms" class="form-control" value="<?php echo set_value('payment_terms',$quotation->payment_terms); ?>" />
								</div>
                            </div>
                            <div class="form-group">
								<label class="col-sm-2 control-label col-lg-2" for="machine_name">Machine Name</label>
								<div class="col-lg-4">
									<input type="text" name="machine_name" id="machine_name" class="form-control" value="<?php echo set_value('machine_name',$quotation->machine_name); ?>" />
								</div>
								
								<label class="col-sm-2 control-label col-lg-2" for="comment">Comment</label>
                                <div class="col-lg-4">
                                    <input type="text" id="comment" name="comment" class="form-control" value="<?php echo set_value('comment',$quotation->comment); ?>" />
                                </div>
                            </div>
							<div class="form-group">
                                    ...
                                    <tbody>
                                        ...
                                        <tr id="<?php echo $count++; ?>">
                                            ...
                                            <?php
                                                ...
                                                $product_id = $count-1;
                                                echo form_dropdown("parts_product_id$product_id",$optionParts,$row->part_id,'class="form-control chosen-select" id="parts_product_id-'.$product_id.'" onchange="getParts(this);"');
                                            ?>
                                            </td>
                                            <td>
                                                <input type="text" class="form-control" placeholder="Description" id="description-<?php echo ($count-1); ?>" name="description<?php echo ($count-1); ?>" value="<?php echo set_value('description'.($count-1),$row->description) ?>" />
                                            </td>
                                            <td>
                                                <input type="text" class="form-control" placeholder="Quantity" id="quantity-<?php echo ($count-1); ?>" name="quantity<?php echo ($count-1); ?>" value="<?php echo set_value('quantity'.($count-1),$row->quantity) ?>" onkeyup="calculateSubTotal(this);" >
                                            </td>
                                            <td>
                                                <input type="text" class="form-control" placeholder="Price" id="unit_price-<?php echo ($count-1); ?>" name="unit_price<?php echo ($count-1); ?>" value="<?php echo set_value('unit_price'.($count-1),$row->price); ?>" onkeyup="calculateSubTotal(this);" >
                                            </td>    
                                            <td>
                                                <?php
                                                    ...
                                                    if(!empty($discount)){
                                                        ...
                                                        $discount_val = $count-1;
                                                        echo form_dropdown("discount_amount$discount_val",$optionDisc,$row->discount_type,'class="form-control" onchange="calculateSubTotal(this);" id="discount_amount-'.$discount_val.'"');
                                                    }
                                                ?>
                                            </td>
                                            <td>
                                                <input type="text" class="form-control" placeholder="Total Price" id="total_price-<?php echo ($count-1); ?>" name="total_price<?php echo ($count-1); ?>" value="<?php echo set_value('total_price'.($count-1),$row->total_price); ?>" onkeyup="calculateSubTotal(this);" disabled>
                                            </td>
                                            <td>
                                                <input type="text" class="form-control form-control-inline input-medium default-date-picker" placeholder="Delivery Date" id="delivery_date-<?php echo ($count-1); ?>" name="delivery_date<?php echo ($count-1); ?>" value="<?php echo set_value('delivery_date'.($count-1),date('d/m/Y',strtotime($row->delivery_date))); ?>" >
                                            </td>
                                            <td>
                                                <input type="text" class="form-control sub_total" placeholder="Sub Total" id="grand_total-<?php echo ($count-1); ?>" name="grand_total<?php echo ($count-1); ?>" value="<?php echo set_value('grand_total'.($count-1),$row->sub_total); ?>" disabled>
                                            </td>
                                            <td>
                                            ...
                                            <input type="hidden" name="disc_value<?php echo ($count-1); ?>" id="disc_value-<?php echo ($count-1); ?>" value="<?php echo $row->discount_value; ?>">
                                            <input type="hidden" name="total_price_hidden<?php echo ($count-1); ?>" id="total_price_hidden-<?php echo ($count-1); ?>" value="<?php echo $row->total_price; ?>">
                                        </tr>
                                        <?php
                                                }
                                            }
                            ...
                            <input type="hidden" name="grand_total" id="grand_total" value="<?php echo $quotation->total; ?>"/>
